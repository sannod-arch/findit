...
const LT = {
  ...
  uploadSub: "Paspauskite, kad pasirinktumėte nuotrauką arba padarytumėte naują",
  ...
  name: "Pavadinimas *",
  descriptionPh: "Aprašymas (spalva, požymiai, prekės ženklas) *",
  ...
  aiSuggests: "AI siūlo klausimus:", clickToSelect: "Paspauskite norėdami pasirinkti",
  ...
  secretQuestionLabel: "Slaptasis klausimas", sendAnswer: "Siųsti atsakymą",
  ...
  geoDetecting: "Nustatoma vieta...", geoDetected: "Vieta nustatyta",
  ...
  photosLabel: "Nuotraukų galerija (iki 4)", photoAdd: "+ Nuotrauka", photoMax: "Maks. 4 nuotraukos",
  // Matching
  matchingTitle: "Galimi sutapimai",
  matchingSub: "Jūsų skelbimai palyginti su kitų vartotojų skelbimais",
  matchSearch: "Ieškoti sutapimų",
  matchSearching: "Ieškoma...",
  ...
  matchAlgo: "Kaip veikia algoritmas",
  ...
  matchEmptySub: "Pridėkite savo skelbimą arba palaukite, kol kiti pridės",
  matchNewBanner: "✦ Naujas sutapimas!",
  matchScore: "balas",
...
const SECRET_QUESTIONS = {
  electronics: ["Kokios spalvos buvo įrenginio dėklas?", "Ar ekrane buvo įtrūkimų ar įbrėžimų?", "Kokie paskutiniai 4 serijos numerio skaitmenys?", "Ar buvo apsauginis stiklas?"],
  documents:   ["Kokia dokumento išdavimo data?", "Koks pirmas asmenvardžio raidė?", "Ar dokumente buvo kitų kortelių?"],
  keys:        ["Kiek raktų buvo ryšulyje?", "Ar buvo pakabukas? Koks?", "Kokios spalvos buvo raktų laikiklis?", "Ar buvo automobilio raktas?"],
  bags:        ["Kas buvo viduje krepšyje?", "Kokios spalvos buvo pamušalas viduje?", "Kiek kišenių turėjo krepšys?"],
  animals:     ["Kokios spalvos buvo antkaklis?", "Ar gyvūnas sterilizuotas?", "Koks gyvūno vardas?", "Ar yra ypatingų požymių?"],
...
// Demo vartotojo ID (realiai būtų iš auth)
const MY_USER_ID = "user_me";

const DEMO_ITEMS = [
  { id: 1, userId: MY_USER_ID, type: "found", category: "animals", title: "Rasta balta katė", description: "Balta katė su rudais dėmiais, labai baikšti, be antkaklio", location: "Žirmūnai, kiemas prie nr.12", city: "Vilnius", country: "lt", date: "Kovo 28", blurred: false, photos: [], tags: ["katė", "balta", "ruda", "baikšti"], geoPin: { lat: 54.712, lng: 25.297 } },
  { id: 2, userId: MY_USER_ID, type: "lost", category: "keys", title: "Pametami raktai su BMW pakabuku", description: "BMW automobilio raktas + 2 namų raktai, rudas odinis pakabuko laikiklis", location: "Akropolis PC", city: "Vilnius", country: "lt", date: "Kovo 27", blurred: false, photos: [], tags: ["raktai", "BMW", "pakabuko", "odinis"], geoPin: { lat: 54.722, lng: 25.301 } },
  { id: 3, userId: "user_other1", type: "found", category: "animals", title: "Rasta balta katė su dėmėmis", description: "Balta katė su rudomis dėmėmis, be antkaklio, rasta parke", location: "Vingio parkas", city: "Vilnius", country: "lt", date: "Kovo 29", blurred: false, photos: [], tags: ["katė", "balta", "ruda", "dėmės"], geoPin: { lat: 54.692, lng: 25.258 } },
  { id: 4, userId: "user_other2", type: "found", category: "keys", title: "Rasti raktai prie parduotuvės", description: "Raktų ryšulys su automobiliniais raktais ir juodu pakabuku", location: "Ozas PC, automobilių stovėjimas", city: "Vilnius", country: "lt", date: "Kovo 26", blurred: false, photos: [], tags: ["raktai", "automobilis", "pakabuko", "juodas"], geoPin: { lat: 54.728, lng: 25.267 } },
  { id: 5, userId: "user_other3", type: "lost", category: "electronics", title: "Pamesta pilka iPad mini", description: "iPad mini skaidriame dėkle su katės lipduku", location: "Telegrafas restoranas", city: "Vilnius", country: "lt", date: "Kovo 28", blurred: false, urgent: true, photos: [], tags: ["ipad", "planšetė", "pilka", "lipduka"] },
  { id: 6, userId: MY_USER_ID, type: "lost", category: "electronics", title: "Pamesta juoda išmanoji", description: "Juodas telefonas juodame dėkle, ekrano apačia įtrūkusi", location: "Vilniaus autobusų stotis", city: "Vilnius", country: "lt", date: "Kovo 25", blurred: false, photos: [], tags: ["telefonas", "juodas", "dėklas", "įtrūkimas"] },
];
...
// ─── EXIF GPS — TIKRAS PARSERIS ───────────────────────────────────────────────
// Naudojame exifr biblioteką iš CDN (įkeliama dinamiškai)
async function loadExifr() {
  ...
  return new Promise((resolve, reject) => {
    ...
    s.onerror = () => reject(new Error("Nepavyko įkelti exifr"));
...
async function classifyImageWithGemini(base64Image, mimeType, itemType = "found") {
  ...
  const prompt = `Tu padedi klasifikuoti rastus/pamestus daiktus skaitmeniniame radinių biure. Analizuok paveikslėlį ir grąžink TIK JSON be markdown:
{
  "category": "electronics|documents|keys|bags|clothing|animals|jewelry|other",
  "titleLt": "skelbimo pavadinimas lietuviškai (${typeText} daiktas, maks 5 žodžiai, pvz 'Rasta juoda katė')",
  "description": "išsamus aprašymas lietuviškai: spalva, forma, požymiai (2-3 sakiniai)",
  "color": "pagrindinė spalva lietuviškai",
  "brand": "prekės ženklas jei matomas arba 'nežinomas'",
  "condition": "naujas|gera|naudotas|pažeistas",
  "tags": ["raktiniai", "žodžiai", "lietuviškai"],
  "confidence": 85,
  "blur_suggestion": "ką užmaskuoti dėl konfidencialumo arba tuščia eilutė",
  "secretQuestions": ["3 konkretūs slaptieji klausimai apie šį daiktą"]
}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`API ${response.status}: ${e?.error?.message || "Klaida"}`);
  }
  ...
  try {
    ...
    if (!parsed.secretQuestions?.length)
...
function calcTagScore(tags1 = [], tags2 = []) {
  if (!tags1.length || !tags2.length) return 0;
  const s1 = new Set(tags1.map(t => t.toLowerCase()));
  const s2 = new Set(tags2.map(t => t.toLowerCase()));
  let matches = 0, partials = 0;
  s1.forEach(t1 => {
    if (s2.has(t1)) { matches++; return; }
    s2.forEach(t2 => { if (t1.includes(t2) || t2.includes(t1)) partials++; });
  });
  return Math.min(1, (matches + partials * 0.5) / Math.max(s1.size, s2.size));
}
...
function matchItems(myItems, allItems) {
  ...
  myItems.forEach(mine => {
    ...
    others.forEach(other => {
      const lostItem  = mine.type === "lost"  ? mine : other;
      const foundItem = mine.type === "found" ? mine : other;

      // Kategorija — 25
      const catScore = 25;

      // Atstumas — 30
      let distScore = 15; // default jei nėra koordinačių
      let distMeters = null;
      if (lostItem.geoPin && foundItem.geoPin) {
        distMeters = calcDistance(lostItem.geoPin.lat, lostItem.geoPin.lng, foundItem.geoPin.lat, foundItem.geoPin.lng);
        distScore = distMeters < 200 ? 30 : distMeters < 500 ? 25 : distMeters < 1000 ? 20 : distMeters < 2000 ? 14 : distMeters < 5000 ? 8 : 3;
      }

      // Data — 20
      const diffDays = Math.abs(new Date(lostItem.date) - new Date(foundItem.date)) / 86400000;
      const dateScore = diffDays <= 1 ? 20 : diffDays <= 3 ? 16 : diffDays <= 7 ? 10 : diffDays <= 14 ? 5 : 2;

      // Aprašymas / tagai — 25
      const tagScore = Math.round(calcTagScore(lostItem.tags, foundItem.tags) * 25);

      const total = catScore + distScore + dateScore + tagScore;
      if (total < 35) return;

      const matchedTags = (lostItem.tags || []).filter(t =>
        (foundItem.tags || []).some(ft => ft.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(ft.toLowerCase()))
      );

      results.push({
        id: `${mine.id}-${other.id}`,
        myItem: mine, otherItem: other,
        lostItem, foundItem,
        score: total,
        matchedTags,
        breakdown: {
          category: { score: catScore, max: 25, label: LT.matchCatLabel },
          distance: { score: distScore, max: 30, label: LT.matchDistLabel, detail: distMeters ? `${(distMeters / 1000).toFixed(1)} km` : "—" },
          date:     { score: dateScore, max: 20, label: LT.matchDateLabel, detail: `${Math.round(diffDays || 0)} d.` },
          tags:     { score: tagScore,  max: 25, label: LT.matchDescLabel, detail: `${matchedTags.length} sutamp.` },
        }
      });
    });
  });
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)", flexShrink: 0 }}>
      ...
      <text x={size/2} y={size/2+1} textAnchor="middle" dominantBaseline="middle"
        style={{ transform: `rotate(90deg)`, transformOrigin: `${size/2}px ${size/2}px`, fontSize: size*0.22, fontWeight: 800, fill: color, fontFamily: "monospace" }}>
        {score}
      </text>
    </svg>
  );
...
function Lightbox({ photos, startIdx, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.95)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <button onClick={onClose} style={{ position: "absolute", top: "16px", right: "16px", background: "rgba(255,255,255,0.1)", border: "none", color: "#fff", width: "36px", height: "36px", borderRadius: "50%", cursor: "pointer", fontSize: "18px" }}>✕</button>
      <div onClick={e => e.stopPropagation()} style={{ maxWidth: "92vw", maxHeight: "88vh", position: "relative" }}>
        ...
        {photos.length > 1 && (
          <>
            {idx > 0 && <button onClick={() => setIdx(i => i - 1)} style={{ position: "absolute", left: "-44px", top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", width: "36px", height: "36px", borderRadius: "50%", cursor: "pointer", fontSize: "18px" }}>‹</button>}
            {idx < photos.length - 1 && <button onClick={() => setIdx(i => i + 1)} style={{ position: "absolute", right: "-44px", top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", width: "36px", height: "36px", borderRadius: "50%", cursor: "pointer", fontSize: "18px" }}>›</button>}
            <div style={{ position: "absolute", bottom: "10px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "5px" }}>
              {photos.map((_, i) => <div key={i} onClick={() => setIdx(i)} style={{ width: i === idx ? "16px" : "6px", height: "6px", borderRadius: "3px", background: i === idx ? "#fff" : "rgba(255,255,255,0.4)", cursor: "pointer", transition: "all 0.2s" }} />)}
            </div>
          </>
        )}
      </div>
  ...
  return (
    <>
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
        {photos.map((src, i) => (
          <div key={i} style={{ position: "relative", width: "80px", height: "80px", borderRadius: "10px", overflow: "hidden", flexShrink: 0, cursor: "pointer" }}>
            <img src={src} alt="" onClick={() => setLightboxIdx(i)} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            ...
            {i === 0 && <div style={{ position: "absolute", bottom: "3px", left: "3px", background: "rgba(99,102,241,0.88)", borderRadius: "3px", fontSize: "8px", color: "#fff", padding: "1px 4px", fontWeight: "700" }}>AI</div>}
          </div>
        ))}
        {photos.length < maxPhotos && (
          <div onClick={() => inputRef.current?.click()} style={{ width: "80px", height: "80px", borderRadius: "10px", border: "2px dashed rgba(255,255,255,0.2)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", gap: "3px", flexShrink: 0, transition: "all 0.15s" }} onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(99,102,241,0.6)"} onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.2)"}>
            <span style={{ fontSize: "22px", color: G.muted }}>+</span>
            <span style={{ fontSize: "9px", color: G.muted }}>nuotr.</span>
          </div>
        )}
      </div>
      <div style={{ fontSize: "10px", color: G.dim, marginTop: "6px" }}>{LT.photoMax} · Paspauskite nuotrauką norėdami padidinti</div>
      {/* Input be capture atributo — leidžia rinktis iš galerijos arba kamerai */}
...
function LeafletMap({ pin, buffer, onPinChange, interactive = true, height = 200 }) {
  ...
  useEffect(() => {
    const initMap = () => {
      ...
      if (interactive && onPinChange) {
        map.on("click", e => onPinChange({ lat: e.latlng.lat, lng: e.latlng.lng }));
      }
    };
  ...
  useEffect(() => {
    ...
    if (interactive && onPinChange) {
      markerRef.current.on("dragend", e => { const p = e.target.getLatLng(); onPinChange({ lat: p.lat, lng: p.lng }); });
    }
    if (buffer) {
      circleRef.current = L.circle([pin.lat, pin.lng], { radius: buffer, color: G.warn, fillColor: G.warn, fillOpacity: 0.12, weight: 1.5, dashArray: "5,5" }).addTo(map);
    }
    map.setView([pin.lat, pin.lng], 15);
  }, [pin, buffer]);

  return <div id={containerId} style={{ width: "100%", height: `${height}px`, borderRadius: "12px", overflow: "hidden", background: "#0e1621" }} />;
}

// ─── GEOLOKACIJOS ŽINGSNIS ────────────────────────────────────────────────────
function GeoStep({ onDone, onBack, photoFile }) {
  ...
  const detectGps = () => {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      ...
      setPin({ lat, lng }); setAddressText(addr);
      setDetecting(false); setPhase("map");
    }, err => {
      setGeoError("Nepavyko: " + (err.code === 1 ? "Leidimai atmesti" : err.message));
      setDetecting(false);
  ...
  const detectExif = async () => {
    ...
    if (coords) {
      ...
      setPin(coords); setAddressText(addr); setPhase("map");
    } else {
  ...
  const confirm = () => onDone({ pin, buffer: buffer || null, address: addressText });
  ...
  if (phase === "source") return (
    <div>
      ...
      {[
        ...
        <div key={opt.id} onClick={() => handleSource(opt.id)}
  ...
  return (
    <div>
      <div style={{ marginBottom: "10px" }}>
        <LeafletMap pin={pin} buffer={buffer || 0} onPinChange={handlePinChange} interactive={true} height={200} />
        {!pin && <div style={{ fontSize: "11px", color: G.muted, textAlign: "center", marginTop: "6px" }}>{LT.geoSetPin}</div>}
      </div>
      {pin && (
        <>
          <div style={{ display: "flex", gap: "6px", marginBottom: "10px" }}>
            <div style={{ flex: 1, background: "rgba(255,255,255,0.05)", borderRadius: "8px", padding: "7px 11px", fontSize: "11px", color: G.muted, fontFamily: "monospace" }}>
              📍 {addressText || `${pin.lat.toFixed(4)}, ${pin.lng.toFixed(4)}`}
            </div>
            <button onClick={() => { setPin(null); setAddressText(""); }} style={{ background: "rgba(231,76,60,0.12)", border: "1px solid rgba(231,76,60,0.3)", borderRadius: "8px", color: G.lost, padding: "7px 11px", fontSize: "11px", cursor: "pointer" }}>{LT.geoReset}</button>
          </div>
          ...
          <div style={{ display: "flex", gap: "6px", marginBottom: "12px" }}>
            {[["exact", LT.geoExact, true], ["100", LT.geo100, false], ["500", LT.geo500, false]].map(([id, label, warn]) => (
              <button key={id} onClick={() => handleBufferChoice(id)} style={{ flex: 1, background: bufferChoice === id ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${bufferChoice === id ? "rgba(255,255,255,0.35)" : G.border}`, borderRadius: "8px", color: bufferChoice === id ? G.text : G.muted, padding: "8px 4px", fontSize: "10px", fontWeight: bufferChoice === id ? "700" : "400", cursor: "pointer", lineHeight: 1.3, textAlign: "center" }}>
                {label}
                {warn && <span style={{ display: "block", fontSize: "9px", color: G.lost }}>⚠ {LT.geoExactWarn}</span>}
              </button>
            ))}
          </div>
        </>
      )}
      <input value={addressText} onChange={e => setAddressText(e.target.value)} placeholder={LT.geoAddressPh}
        style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: `1px solid ${G.border}`, borderRadius: "9px", padding: "10px 13px", color: G.text, fontSize: "13px", outline: "none", marginBottom: "12px", boxSizing: "border-box" }} />
      <div style={{ display: "flex", gap: "7px" }}>
        ...
        <button onClick={() => (pin || addressText) && confirm()} style={{ flex: 2, background: (pin || addressText) ? G.found : "rgba(255,255,255,0.06)", border: "none", color: (pin || addressText) ? G.text : G.muted, padding: "12px", borderRadius: "10px", fontWeight: "700", fontSize: "13px", cursor: (pin || addressText) ? "pointer" : "default" }}>{LT.geoConfirm}</button>
      </div>
    </div>
  );
}
...
function AIPhotoAnalyzer({ onResult, onSkip, itemType }) {
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      const ai = await classifyImageWithGemini(compressed.split(",")[1], "image/jpeg", itemType);
      ...
      {phase === "idle" && (
        <>
          <div onClick={() => inputRef.current?.click()}
            style={{ border: "2px dashed rgba(99,102,241,0.38)", borderRadius: "16px", padding: "32px 20px", textAlign: "center", cursor: "pointer", background: "rgba(99,102,241,0.05)", marginBottom: "12px" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = G.found; e.currentTarget.style.background = "rgba(99,102,241,0.1)"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(99,102,241,0.38)"; e.currentTarget.style.background = "rgba(99,102,241,0.05)"; }}>
            <div style={{ fontSize: "34px", marginBottom: "10px" }}>⊙</div>
            <div style={{ fontSize: "15px", fontWeight: "700", color: G.text, marginBottom: "5px" }}>{LT.uploadPhoto}</div>
            <div style={{ fontSize: "12px", color: G.muted, lineHeight: 1.5 }}>{LT.uploadSub}</div>
          ...
          {/* BEZ capture atributo — vartotojas pasirenka: galerija arba kamera */}
      ...
      {phase === "result" && result && (
        <div style={{ animation: "pop 0.3s cubic-bezier(0.16,1,0.3,1)" }}>
          <div style={{ position: "relative", marginBottom: "12px" }}>
            <img src={preview} alt="" style={{ width: "100%", maxHeight: "170px", objectFit: "cover", borderRadius: "12px", display: "block" }} />
          ...
          <div style={{ background: catColors?.bg || "#1a1a2e", border: `1px solid ${catColors?.accent || G.found}44`, borderRadius: "13px", padding: "13px", marginBottom: "10px" }}>
            ...
            {result.blur_suggestion && (
              <div style={{ marginTop: "8px", background: "rgba(245,166,35,0.12)", border: "1px solid rgba(245,166,35,0.3)", borderRadius: "7px", padding: "7px 10px", fontSize: "11px", color: G.warn, display: "flex", gap: "6px" }}>
                <span>⚠</span><span>{LT.blurSuggestion} {result.blur_suggestion}</span>
              </div>
            )}
          </div>
...
function AddModal({ onClose, onAdd, defaultType }) {
  ...
  const [form, setForm] = useState({ title: "", description: "", category: "", country: "lt", phone: "", email: "", blurPhoto: false, customQuestion: "" });
  ...
  const handleSubmit = () => {
    ...
    onAdd({
      id: Date.now(), userId: MY_USER_ID, type,
      category: form.category || "other",
      title: form.title, description: form.description,
      location: geoData?.address || "", geoPin: geoData?.pin || null, geoBuffer: geoData?.buffer || null,
      city: loc?.city || "Vilnius", country: form.country,
      date: "Šiandien", blurred: form.blurPhoto,
      ...
      photos, secretQuestion: selectedQuestion || form.customQuestion,
    });
    onClose();
  };
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.86)", backdropFilter: "blur(12px)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: "24px", padding: "26px", maxWidth: "500px", width: "100%", maxHeight: "93vh", overflowY: "auto" }}>
          ...
          <>
            {aiResult && (
              <div style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.25)", borderRadius: "10px", padding: "10px 13px", marginBottom: "12px", display: "flex", gap: "9px", alignItems: "center" }}>
                <span>✦</span>
                <div>
                  <div style={{ fontSize: "10px", color: "#818cf8", marginBottom: "1px", fontWeight: "700" }}>{LT.aiFilledAuto}</div>
                  <div style={{ fontSize: "12px", color: G.text }}>{LT.categories[aiResult.category]} · {aiResult.confidence}%</div>
                </div>
              </div>
            )}
        ...
        {step === 3 && (
          <>
            {!geoData
              ? <GeoStep onDone={d => setGeoData(d)} onBack={() => setStep(2)} photoFile={rawFile} />
              : (
                <div>
          ...
          <>
            ...
            {type === "found" && (
              <>
                ...
                <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginBottom: "8px" }}>{LT.aiSuggests} {LT.clickToSelect}</div>
                <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "10px" }}>
                  {availableQuestions.map((q, i) => (
                    <div key={i} onClick={() => setSelectedQuestion(q === selectedQuestion ? "" : q)}
                      style={{ background: selectedQuestion === q ? "rgba(99,102,241,0.18)" : "rgba(255,255,255,0.03)", border: `1px solid ${selectedQuestion === q ? G.found : G.border}`, borderRadius: "9px", padding: "10px 13px", cursor: "pointer", display: "flex", gap: "10px", alignItems: "flex-start" }}>
                      <div style={{ width: "15px", height: "15px", borderRadius: "50%", border: `2px solid ${selectedQuestion === q ? G.found : "rgba(255,255,255,0.25)"}`, background: selectedQuestion === q ? G.found : "transparent", flexShrink: 0, marginTop: "1px" }} />
                      <span style={{ fontSize: "12px", color: selectedQuestion === q ? G.text : G.muted, lineHeight: 1.5 }}>{q}</span>
                    </div>
                  ))}
                </div>
                <input value={form.customQuestion} onChange={e => { setForm(f => ({ ...f, customQuestion: e.target.value })); setSelectedQuestion(""); }} placeholder={LT.ownQuestion} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: `1px solid ${G.border}`, borderRadius: "9px", padding: "10px 13px", color: G.text, fontSize: "13px", outline: "none", marginBottom: "14px", boxSizing: "border-box" }} />
              </>
            )}
            ...
            <div style={{ display: "flex", gap: "7px" }}>
              ...
              <button onClick={handleSubmit} style={{ flex: 2, background: `linear-gradient(135deg, ${accent}, ${type === "found" ? "#8b5cf6" : "#c0392b"})`, border: "none", color: G.text, padding: "13px", borderRadius: "10px", fontWeight: "800", fontSize: "14px", cursor: "pointer" }}>{LT.publish}</button>
            </div>
          </>
        )}
      </div>
          ...
          {photos.length > 1 && (
            <div style={{ display: "flex", gap: "4px", marginBottom: "8px" }}>
              {photos.slice(1, 4).map((p, i) => (
                <img key={i} src={p} alt="" onClick={e => { e.stopPropagation(); setLightboxIdx(i + 1); }}
                  style={{ width: "38px", height: "38px", objectFit: "cover", borderRadius: "6px", cursor: "pointer" }} />
              ))}
            </div>
          )}
        ...
        <div onClick={e => e.stopPropagation()} style={{ background: bg, border: `1px solid ${accent}44`, borderRadius: "24px", width: "100%", maxWidth: "500px", display: "flex", flexDirection: "column", maxHeight: "93vh", overflow: "hidden", boxShadow: "0 40px 80px rgba(0,0,0,0.6)" }}>
          ...
          {photos.length > 0 && (
            <div style={{ position: "relative", cursor: "zoom-in" }} onClick={() => setLightboxIdx(photoIdx)}>
              <img src={photos[photoIdx]} alt="" style={{ width: "100%", height: "210px", objectFit: "cover", display: "block" }} />
              {photos.length > 1 && (
                <>
                  <div style={{ position: "absolute", bottom: "10px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "5px" }}>
                    {photos.map((_, i) => <div key={i} onClick={e => { e.stopPropagation(); setPhotoIdx(i); }} style={{ width: i === photoIdx ? "16px" : "6px", height: "6px", borderRadius: "3px", background: i === photoIdx ? "#fff" : "rgba(255,255,255,0.5)", cursor: "pointer", transition: "all 0.2s" }} />)}
                  </div>
                  <div style={{ position: "absolute", top: "10px", right: "10px", background: "rgba(0,0,0,0.55)", borderRadius: "6px", padding: "3px 8px", fontSize: "11px", color: "rgba(255,255,255,0.8)" }}>{photoIdx + 1}/{photos.length}</div>
                </>
              )}
            </div>
          )}
          ...
          <div style={{ overflowY: "auto", padding: "14px 22px 22px", flex: 1 }}>
            ...
            {tab === "contact" && (
              <>
                ...
                <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: "10px", padding: "10px", minHeight: "86px", marginBottom: "10px", display: "flex", flexDirection: "column", gap: "7px" }}>
...
function MatchingScreen({ items }) {
  ...
  const myItems = items.filter(i => i.userId === MY_USER_ID);
  const matches = useMemo(() => matchItems(myItems, items), [items]);

  const visible = matches
    .filter(m => !dismissed.has(m.id) && m.score >= minScore)
  ...
  return (
    <div style={{ padding: "0 20px 100px" }}>
      ...
      {/* Mygtukas rankiniui ieškojimui */}
      <button onClick={handleSearch} style={{ width: "100%", background: searching ? "rgba(99,102,241,0.15)" : "rgba(99,102,241,0.1)", border: `1px solid ${G.found}44`, borderRadius: "11px", padding: "12px", color: "#818cf8", fontSize: "13px", fontWeight: "700", cursor: "pointer", marginBottom: "14px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
      ...
      {visible.length === 0 ? (
        ...
        const { myItem, otherItem, lostItem, foundItem, score, matchedTags, breakdown } = match;
            ...
            <div onClick={() => setExpandedId(isExpanded ? null : match.id)} style={{ padding: "15px 17px", cursor: "pointer", display: "flex", alignItems: "center", gap: "13px" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", gap: "6px", marginBottom: "6px", alignItems: "center", flexWrap: "wrap" }}>
                  <span style={{ background: `${scoreColor}22`, border: `1px solid ${scoreColor}55`, color: scoreColor, fontSize: "10px", fontWeight: "800", padding: "2px 7px", borderRadius: "4px", letterSpacing: "0.06em" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: "5px", alignItems: "center" }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: "9px", color: myIsLost ? G.lost : G.found, fontWeight: "700", marginBottom: "2px" }}>
                      {myIsLost ? `⚠ ${LT.matchMyLost}` : `◉ ${LT.matchMyFound}`}
                    </div>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: "9px", color: myIsLost ? G.found : G.lost, fontWeight: "700", marginBottom: "2px" }}>
                      {myIsLost ? `◉ ${LT.matchOtherFound}` : `⚠ ${LT.matchOtherLost}`}
                    </div>
              ...
              <div style={{ borderTop: `1px solid ${G.border}`, padding: "14px 17px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "14px" }}>
                  {[[myItem, myIsLost ? G.lost : G.found, myIsLost ? LT.matchMyLost : LT.matchMyFound],
                    ...
                    <div key={item.id} style={{ background: `${color}0d`, border: `1px solid ${color}33`, borderRadius: "10px", padding: "11px" }}>
                      <div style={{ fontSize: "9px", color, fontWeight: "800", marginBottom: "5px", textTransform: "uppercase" }}>{label}</div>
                      {item.photos?.[0] && <img src={item.photos[0]} alt="" style={{ width: "100%", height: "70px", objectFit: "cover", borderRadius: "7px", marginBottom: "7px" }} />}
                      <div style={{ fontSize: "12px", color: G.text, fontWeight: "600", marginBottom: "3px" }}>{item.title}</div>
                      <div style={{ fontSize: "10px", color: G.muted, lineHeight: 1.4 }}>{item.description}</div>
                      <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)", marginTop: "5px" }}>◷ {item.date} · {item.city}</div>
                    </div>
                  ))}
                </div>
                ...
                {matchedTags.length > 0 && (
                  <div style={{ marginBottom: "13px" }}>
                    <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginBottom: "7px", textTransform: "uppercase", letterSpacing: "0.07em" }}>{LT.matchTags}</div>
                ...
                {/* Balo detalizavimas */}
                <div style={{ marginBottom: "13px" }}>
                  <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.07em" }}>Balo detalizavimas</div>
                  {Object.values(breakdown).map(b => {
                    const pct = b.score / b.max;
                    const barColor = pct >= 0.75 ? G.success : pct >= 0.5 ? G.warn : G.lost;
                    return (
                      <div key={b.label} style={{ marginBottom: "8px" }}>
                        ...
                        <div style={{ height: "3px", background: "rgba(255,255,255,0.07)", borderRadius: "2px" }}>
                          <div style={{ height: "100%", width: `${pct * 100}%`, background: barColor, borderRadius: "2px", transition: "width 0.6s ease" }} />
                        </div>
                      </div>
                    );
                  })}
...
function ApiKeyBanner({ onDismiss }) {
  ...
  const save = () => {
    if (key.length > 10) { window.__geminiKey = key; setSaved(true); setTimeout(onDismiss, 900); }
  };
  return (
...
export default function App() {
  ...
  // Automatinis matching kai pridedamas naujas skelbimas
  const handleAdd = (newItem) => {
    setItems(prev => {
      const updated = [newItem, ...prev];
      // Ieškome sutapimų tarp naujų ir esamų
      const newMatches = matchItems([newItem], updated.filter(i => i.id !== newItem.id));
  ...
  const totalMatches = useMemo(() => matchItems(myItems, items).length, [items]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: G.bg, fontFamily: G.sans, color: G.text }}>
      ...
      {screen === "profile" && (
        <div style={{ padding: "20px", color: G.muted, textAlign: "center" }}>
          <div style={{ fontSize: "32px", marginBottom: "12px" }}>▤</div>
          <div style={{ fontSize: "15px", color: G.text, fontWeight: "700" }}>Paskyra</div>
          <div style={{ fontSize: "12px", marginTop: "6px" }}>Netrukus...</div>
        </div>
      )}
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(6,10,15,0.97)", backdropFilter: "blur(16px)", borderTop: "1px solid rgba(255,255,255,0.07)", padding: "10px 20px 18px", display: "flex", gap: "4px" }}>
        {[["feed", "◈", LT.feed], ["matching", "✦", LT.matching], ["profile", "▤", LT.cabinet]].map(([id, icon, label]) => (
          <button key={id} onClick={() => id === "add" ? setShowAdd(true) : setScreen(id)} style={{ flex: 1, background: screen === id ? "rgba(255,255,255,0.08)" : "transparent", border: `1px solid ${screen === id ? "rgba(255,255,255,0.14)" : "transparent"}`, borderRadius: "10px", color: G.text, padding: "8px 6px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", transition: "all 0.2s", position: "relative" }}>
