...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
// Paste your Anthropic API key here for local testing.
// On Vercel: add VITE_ANTHROPIC_KEY in Project Settings → Environment Variables
...
const LOCATIONS = [
  { id: "lt", flag: "🇱🇹", label: "Литва" },
  { id: "lv", flag: "🇱🇻", label: "Латвия" },
  { id: "ee", flag: "🇪🇪", label: "Эстония" },
  { id: "pl", flag: "🇵🇱", label: "Польша" },
  { id: "de", flag: "🇩🇪", label: "Германия" },
];
...
async function classifyImageWithClaude(base64Image, mimeType) {
  if (!API_KEY) {
    ...
    return {
      ...
      brand: "неизвестен",
      condition: "б/у",
      tags: ["смартфон", "электроника", "тёмный"],
      confidence: 87,
...
// ─── AI PHOTO ANALYZER (the star of the show) ─────────────────────────────────
function AIPhotoAnalyzer({ onResult, onSkip }) {
  ...
  const processFile = useCallback(async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        setError("Не удалось распознать. Проверьте API-ключ или попробуйте другое фото.");
        setPhase("error");
      }
    };
  ...
  return (
    <div>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes pulse{0%,100%{opacity:.4}50%{opacity:1}} @keyframes pop{from{transform:scale(.8);opacity:0}to{transform:scale(1);opacity:1}} @keyframes fadeUp{from{transform:translateY(10px);opacity:0}to{transform:translateY(0);opacity:1}}`}</style>
        ...
        <>
          ...
          <div
            ...
            style={{
              ...
              cursor: "pointer", background: dragOver ? "rgba(99,102,241,0.1)" : "rgba(99,102,241,0.05)",
...
function AddModal({ onClose, onAdd, defaultType }) {
  ...
  const aiQ = AI_QUESTIONS[Math.floor(Math.random() * AI_QUESTIONS.length)];
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", backdropFilter: "blur(12px)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: "24px", padding: "28px", maxWidth: "480px", width: "100%", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        {/* STEP 1 — AI Photo */}
        {step === 1 && (
          <AIPhotoAnalyzer onResult={handleAiResult} onSkip={() => setStep(2)} />
        )}
...
function ApiKeyBanner({ onDismiss }) {
  ...
  const save = () => {
    if (key.startsWith("sk-ant-")) {
      sessionStorage.setItem("anthropic_key_demo", key);
      setSaved(true);
  ...
  return (
    <div style={{ margin: "12px 16px", background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: "14px", padding: "16px" }}>
      ...
      <div style={{ fontSize: "10px", color: G.dim, marginTop: "7px", lineHeight: 1.5 }}>
        Ключ хранится только в этом браузере. Получить на <span style={{ color: "#818cf8" }}>console.anthropic.com</span>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: G.bg, fontFamily: G.sans, color: G.text }}>
      ...
      {/* Header */}
      <div style={{ padding: "16px 20px 0", borderBottom: `1px solid rgba(255,255,255,0.05)`, paddingBottom: "14px", position: "sticky", top: 0, background: "rgba(6,10,15,0.97)", backdropFilter: "blur(16px)", zIndex: 100 }}>
